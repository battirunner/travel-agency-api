...
// @access Public
const getInsuranceCategory = asyncHandler(
...
const getInsuranceCategoryById = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const result = await insuranceCategoryService.getInsuranceCategoryById(
      req.params.id
    );
    // generateToken(res, result);
...
const createInsuranceCategory = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const result = await insuranceCategoryService.createInsuranceCategory(
      req.body
    );
    // generateToken(res, result);
...
const updateInsuranceCategory = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    // @ts-ignore
    const result = await insuranceCategoryService.updateInsuranceCategory(
      req.params.id,
      req.body
    );
    res.status(200).json({ data: result });
...
const deleteInsuranceCategory = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const result = await insuranceCategoryService.deleteInsuranceCategory(
      req.params.id
    );
    // generateToken(res, result);
    res.status(200).json({ data: result });
